"""Program to download images from albums in default gallery form IMGUR."""
...
class ImgurDlGallery:
    ...
    def connect_to_imgur_api(self, config_file) -> imgurpython.ImgurClient:
        ...
        try:
            ...
            print(f'Improper credentials passed in config file. Pleae verify the config file: {config_file}.')
            input('Program will now close.')
            quit()
        try: 
            self.read_albums = int(config.get('number of albums', 'no_of_albums'))
        except:
            print(f'Config value for number of albums (no_of_albums) cannot be used. Pleae verify the config file: {config_file}.')
            input('Program will now close.')
            quit()
        return self.client
    ...
    def _download_image(self, image:object, title:str, download_folder:str ) -> None:
        ...
        try:
            ...
            with open(os.path.join(download_folder, f'{title[:20]}.{file_ext}'), 'wb') as saved_image:
    ...
    def download_all_images_from_gallery(self, download_folder:str = os.path.join(os.getcwd(), 'Gallery')) -> None:
        """Downloads albums images available on main Imgur gallery. Assigns file extension. requires API connection. Can accept download directory."""
        items = self.client.gallery()
        os.makedirs(download_folder, exist_ok=True)
        no_name_images = 0
        for index_no, item in enumerate(items):
            if index_no > self.read_albums:                       # <------ download number of albums specified in config file. 
                break   
            if not item.is_album:
                continue
            album_download_folder = os.path.join(download_folder, item.title[:35].strip())
            ...
            for image in self.client.get_album_images(album_id = item.id):
                if image.title:
                    ...
                    no_name_images += 1
                    self._download_image(image, f'unnamed{no_name_images}', album_download_folder)
