...
class Workplace:
    ...
    def __init__(self, workplace_id:int = None, workplace_name:str = None, workplace_address:str = None, 
                 min_employee = 1, max_employee = 5, workplace_schedule:dict = defaultdict()) -> None:
...
class WorkdayCalendar:
    ...
    def initialize_schedule(cls, config, employee_list, workplace_list) -> None:
        ...
        if max_employees < len(employee_list): 
            ...
            print(f'There are {len(employee_list)} employees and {max_employees} workspaces. Minimum required employees: {min_employees} Employees will be assigned to available spaces.' )
            ...
            while workplace_fill_min:
                ...
                for workplace in workplace_fill_min:
                    ...
                    for employee in available_employees:
                        if employee.employee_schedule and list(employee.employee_schedule.keys())[-1] + datetime.timedelta(hours=19) >= day_shift.date + shift_offset[day_shift.shift]:
                            continue
